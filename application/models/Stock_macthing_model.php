...
class Stock_macthing_model extends CI_model {
	public $process_name;
	public $role;
	public $user_id;
	public $table_name;
	...
	//stock matching query
	public function select_stock_matching()
	{
		   $this->db->select('MAX(expt_selling_price) as max_stock, MIN(expt_selling_price) as min_stock');
		   $query = $this->db->from('tbl_stock_in_hand_poc');
		   $query = $this -> db -> get();
	...
	public function downloadLeads() {
		...
		if($role=1 && $getUserid == ''){
				$where="(f2.appointment_type='Showroom Visit' and f2.appointment_date='". $this->today ."')or(f1.appointment_type='Showroom Visit' and f1.appointment_date='". $this->today."')";
			$this -> db -> where($where);
		}
	...
	// Unassign leads
	public function select_lead_location_wise($max_price,$min_price) {
		...
		$this -> db -> select('
			...
			f2.date as dse_date,f2.nextfollowupdate as dse_nfd,f2.nextfollowuptime as dse_nftime,f2.comment as dse_comment,
			l.days60_booking,
			l.assign_by_cse_tl,l.assign_to_cse,l.assign_to_dse,assign_to_dse_tl,l.cse_followup_id,l.dse_followup_id,l.lead_source,l.eagerness,l.enq_id,name,l.email,contact_no,enquiry_for,l.created_date,l.budget_from,l.budget_to,l.make_id,l.nextAction,l.feedbackStatus');
		...
		$this -> db -> where('l.process', $this -> process_name);
		//echo $max_price;
		$this -> db -> where('l.budget_to >=', $min_price);
		$this -> db -> where('l.budget_from <=', $max_price);
	//	$this -> db -> where('l.nextAction', "Showroom Visit");
		if ($role == 5 && $getUserid == '') {
			$this -> db -> where("l.assign_to_dse", $userid);
			//$this -> db -> where('f2.appointment_type', "Stock Matching");
			//$this -> db -> where('f2.appointment_date', $this->today);
		//	$this -> db -> where('f2.nextfollowupdate', $this->today);
			...
			$this -> db -> where("l.assign_to_cse=", $userid);
			//$this -> db -> where('f1.appointment_type', "Stock Matching");
			//$this -> db -> where('f1.appointment_date', $this->today);
			//$this -> db -> where('f1.nextfollowupdate', $this->today);
		...
		// if($role=1 && $getUserid == ''){
		// 		$where="((f2.appointment_type='Stock Matching' and f2.appointment_date='". $this->today ."')or(f1.appointment_type='Stock Matching' and f1.appointment_date='". $this->today."'))";
		// 	$this -> db -> where($where);
		// }
	...
	public function select_lead_count_location_wise($max_price,$min_price) {
