   ...
   public function select_fields() 
	 ...
	 public function insert_mail_template() 
	{
		...
		echo $subject=$this->input->post('subject');
			$query=$this->db->query("select * from tbl_mail_template where template_name='$name' and process_id='$this->process_id'")->result();
		...
		if(count($query)>0)

         {
			...
			$query1=$this ->db->query("select * from tbl_mail_template where template_name='$name'  AND status='-1' and process_id='$this->process_id' ")->result(); 
			 ...
			 $query=$this->db->query("update tbl_mail_template  set status='1'   where template_name='$name' AND status='-1' and process_id='$this->process_id'");
		...
		//	echo 'if'; 
			...
			$query=$this->db->query("insert into tbl_mail_template (template_name,subject,description,status,created_time,created_date,process_id)
			values('$name','$subject','$description','1','$this->time','$this->today','$this->process_id')");
...
//	print_r($images);
...

	{
		...
		$this -> db -> select('m.*, m1.attachment_name, m1.attach_id');//m&m1 are the table name
...

	{
		...
		$query=$this->db->query("select * from tbl_mail_template where template_name='$name'  AND t_id !='$ctype_id' and process_id='$this->process_id'")->result();
		...
		if(count($query)>0)
         ...
         $this -> session -> set_flashdata('message', '<div class="alert alert-danger"><strong>Already Exists ...!</strong>	<a href="#" class="close" data-dismiss="alert" aria-label="close">×</a>	</div>');
			...
			$insert=$this->db->query("UPDATE tbl_mail_template set template_name= '$name', subject= '$subject', description='$description' where t_id= '$ctype_id'");
            	 $i=0;
	    $dataid=1;
	    $t_id=$ctype_id;
	    	$images=array();
