...
class Employment_type_model extends CI_Model {
	...
	public function insert_l_status($login_status) {
     //   $scheme_name=$this->input->post('l_status_name');
		$q = $this -> db -> query('select * from tbl_employment_type where  e_name="'. $login_status .'" ') -> result();
		//print_r($q);

		if (count($q) > 0) {

			$query1=$this->db->query("select * from  tbl_employment_type where e_name='$login_status'  AND  status='-1'")->result();
			 if(count($query1)>0)
			 {
			 
			 $query=$this->db->query("update tbl_employment_type set status='1' where e_name='$login_status'    AND status='-1' ");
			...
			$query = $this -> db -> query("insert into tbl_employment_type (`e_name`,`status`)values('$login_status','1')");
	...
	public function select_l_status() {
		$this -> db -> select('l.*');
		$this -> db -> from('tbl_employment_type l');
  ...
  $scheme_name=$this->input->post('scheme_name');
		$q = $this -> db -> query('select * from tbl_employment_type where e_name="' . $reason_name . '" and  e_id !="'.$login_status_id.'" ') -> result();
		...
		if (count($q) > 0) {
			...
			$this -> db -> query('update tbl_employment_type set e_name="' . $reason_name . '"  where e_id="' . $login_status_id . '"');
	...
	public function delete_l_status($id) {
		...
		$this -> db -> query('update tbl_employment_type set status="-1" where e_id="' . $id . '"');
		if ($this -> db -> affected_rows() > 0) {
