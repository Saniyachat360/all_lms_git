...
class CSE_reports_assign_model extends CI_model
{
	...
	public function cse_performance($cse_id, $fromdate, $todate)
	{
		...
		if (count($query) > 0) {
			...
			foreach ($query as $row) {
				$assign_leads = $this->cse_assigned_leads($row->id, $fromdate, $todate);
				$assign_leads_data = json_encode($assign_leads);

				$total_connected = $this->total_connected($row->id, $fromdate, $todate, 'Connected');
				$total_connected_data = json_encode($total_connected);

				$total_intrested = $this->total_intrested($row->id, $fromdate, $todate, 'Interested');
				$total_intrested_data = json_encode($total_intrested);

				$test_drive = $this->test_drive($row->id, $fromdate, $todate);
				$test_drive_data = json_encode($test_drive);

				
				$booked_leads = $this->cse_booked_leads($row->id, $fromdate, $todate);
				$booked_leads_data = json_encode($booked_leads);
				
				// 'pending_new_leads' => $pending_new_leads,
				$select_data[] = array(
					'cse_id' => $row->id, 'cse_fname' => $row->fname, 'cse_lname' => $row->lname, 'assign_lead' => $assign_leads_data,'booked_leads' => $booked_leads_data, 'test_drive' => $test_drive_data,  'total_connected' => $total_connected_data, 'total_intrested' => $total_intrested_data
				);
			}
		} else {
	...
	{
		$this->db->select('assign_to_cse_date, `l`.`assign_to_cse_date`, 
		COUNT(*) AS count');
		$this->db->from($this->table_name);
		...
		$this->db->group_by('l.assign_to_cse_date');
		$query = $this->db->get();
		echo $this->db->last_query();
		$query1 = $query->result();
		return $query1;
	...
	public function total_intrested($id, $from_date, $to_date, $feedbackStatus)
	{
		$this->db->select('assign_to_cse_date, `l`.`assign_to_cse_date`, 
		COUNT(*) AS count');
		$this->db->from($this->table_name);
		...
		$this->db->where('f.assign_to', $id);
		$this->db->where('f.feedbackStatus', $feedbackStatus);
		...
		//echo $this->db->last_query();
		// $query = $query->result();
		// $total_intrested_count = $query[0]->leadCount;
		// return $total_intrested_count;
	...
	public function cse_assigned_leads($id, $fromdate, $todate)
	{
		...
		if ($this->process_id == 8) {
			...
			if (isset($this->process_name) && is_string($this->process_name)) {
				$this->db->where('l.process', $this->process_name);
			} else {
				throw new Exception("Process name is not set or is not a string.");
			}
		}
	...
	public function cse_booked_leads($id, $fromdate, $todate)
	{
		...
		$this->db->where('l.nextAction', 'Booked');
		$this->db->where('l.assign_to_cse_date>=', $fromdate);
		$this->db->where('l.assign_to_cse_date<=', $todate);
		$query = $this->db->get();
		//echo $this->db->last_query();
	...
	public function test_drive($id, $fromdate, $todate)
	{
		$this->db->select('count(enq_id) as close');
		$this->db->from($this->table_name);
		$this->db->where('l.assign_to_cse', $id);
