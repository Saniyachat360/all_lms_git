...
class New_notification_model_dash extends CI_Model {
	public $process_name;
	public $role;
	public $user_id;
	public 	$location;
...
public function select_default_close_lead_status()
	...
	public function location_data($location_name){
			...
			foreach($query as $row){
				...
				$con1='Home Visit';
				$home_visit_count = $this -> check_count($con1,$con, $row -> id,$row->role);
				
				$con = 'f.appointment_type="Test Drive"';
				$test_drive_count = $this -> check_count($con1,$con, $row -> id,$row->role);
				
				$con = 'f.appointment_type="Showroom Visit"';
				$con1='Showroom Visit';
				$showroom_visit_count = $this -> check_count($con1,$con, $row -> id,$row->role);
				...
				// , 'home_visit_count' => $home_visit_count, 'test_drive_count' => $test_drive_count,'showroom_visit_count' => $showroom_visit_count,'evaluation_count' => $evaluation_count,
				// 'escalation_level_1' => $escalation_level_1, 'escalation_level_2' => $escalation_level_2	, 'escalation_level_3' => $escalation_level_3);
			
                // New query optimised with escalation 			
			 ...
			 //   'home_visit_count' => $home_visit_count, 'test_drive_count' => $test_drive_count,'showroom_visit_count' => $showroom_visit_count,'evaluation_count' => $evaluation_count,
			    ...
			    $select_leads[] = array('id'=>$row -> id,'role'=>$row->role, 'location_name'=>$row->location_name,'fname' => $row -> fname, 'lname' => $row -> lname,
			    'home_visit_count' => $home_visit_count, 'test_drive_count' => $test_drive_count, 'showroom_visit_count' => $showroom_visit_count);
