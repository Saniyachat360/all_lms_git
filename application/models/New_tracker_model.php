...
class new_tracker_model extends CI_model
{
	function __construct()
	{
		...
		,l.appointment_type,l.appointment_date,l.appointment_time,l.appointment_status,l.pickup_required,l.service_type';
			if ($this->process_id == 1) {
				$this->selectElement = '`tloc`.`location` as `showroom_location`, csef.file_login_date,csef.pick_up_date,csef.login_status_name,csef.approved_date,csef.disburse_date,csef.disburse_amount,csef.process_fee,csef.emi,csef.eagerness,l.bank_name,l.loan_type,l.los_no,l.roi,l.tenure,l.loanamount,l.dealer,l.executive_name
		,l.appointment_type,l.appointment_date,l.appointment_time,l.appointment_status,l.pickup_required,l.service_type,lfin.*,efin.e_name,pfin.p_type_name';
			}
		}
	...
	public function select_leads()
	{
		...
		l.reg_no, min(l.created_date) as min_date, max(l.created_date) as max_date, l.dms_enq_number, l.followup_fuel_type, l.followup_stock, l.km,`l`.`buyer_type`,  `l`.`manf_year`,l.assign_to_cse_date,l.assign_to_cse_time,
		l.appointment_type,l.appointment_date,l.appointment_time,l.appointment_status,
		...
		mr.followup_pending,mr.call_received,mr.fake_updation,mr.service_feedback
		...
		if ($_SESSION['process_id'] == 1) {
			...
			$query = $query . " LEFT JOIN lead_followup_finance lfin ON `lfin`.`enq_id`=`l`.`enq_id` ";
			$query = $query . " LEFT JOIN tbl_employment_type efin ON `efin`.`e_id`=`lfin`.`emp_type` ";
			$query = $query . " LEFT JOIN tbl_professional_type pfin ON `pfin`.`p_type_id`=`lfin`.`prof_type` ";
		}
		if ($_SESSION['process_id'] == 8) {
	...
	public function select_lead_download_all()
	{
		...
		l.enq_id,l.lead_source,l.enquiry_for,l.process,l.name,l.contact_no,l.alternate_contact_no,l.address,l.email,l.created_date as lead_date,l.created_time as lead_time,l.feedbackStatus,l.nextAction,
		 l.ownership,l.budget_from,l.budget_to,l.accidental_claim,`l`.`assign_by_cse_tl`,`l`.`assign_to_cse`,
		l.reg_no, min(l.created_date) as min_date, max(l.created_date) as max_date,l.dms_enq_number, l.followup_fuel_type, l.followup_stock,  l.km,`l`.`buyer_type`,  `l`.`manf_year`,l.assign_to_cse_date,l.assign_to_cse_time,l.assign_to_dse_date,l.assign_to_dse_time,l.assign_to_dse_tl_date,l.assign_to_dse_tl_time,
		...
		if ($_SESSION['process_id'] == 1) {
			...
			$query = $query . " LEFT JOIN tbl_employment_type efin ON `efin`.`e_id`=`lfin`.`emp_type` ";
			$query = $query . " LEFT JOIN tbl_professional_type pfin ON `pfin`.`p_type_id`=`lfin`.`prof_type` ";
			$query = $query . "LEFT JOIN `tbl_manager_process` `mp` on `mp`.`user_id` =`ucse`.`id`
		LEFT JOIN `tbl_location` `tloc` ON `tloc`.`location_id`= `mp`.`location_id`";
		}
		if ($_SESSION['process_id'] == 8) {
