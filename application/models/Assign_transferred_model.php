...
class Assign_transferred_model extends CI_model {
	function __construct() {
		...
		if ($this -> process_id == 6 || $this -> process_id == 7 ) {
			...
			$this -> table_name2 = 'lead_followup_evaluation';
			
			$this -> table_name1 = 'request_to_lead_transfer_evaluation';
	...
	function mapdse_names() {
		$user_id = $_SESSION['user_id'];
		$this -> db -> select('dse_id');
		$this -> db -> from('tbl_mapdse');
		$this -> db -> where('tl_id', $user_id);
		...
		if (count($query2) > 0) {
			...
			for ($i = 0; $i < $c; $i++) {
				if ($i == 0) {
					...
					$t1 = $t1 . "id = '" . $query2[$i] -> dse_id . "'";

				} else {
					$t1 = $t1 . " or id ='" . $query2[$i] -> dse_id . "'";
				}
			}
			$st1 = $t1 . ')';
...
function all_count() {
	...
	function campaign_name() {
		...
		$this -> db -> select('lead_source,enquiry_for,count(lead_source) as wcount,loan_type');
		$this -> db -> from($this -> table_name);
		$this -> db -> where($assign);
		//$this -> db -> where($nextaction);
		if($this->process_id==8)
	...
	function assign_data_admin() {
		...
		if ($lead_count != '') {
			...
			for ($i = 0; $i < $cse_count; $i++) {
				...
				$this -> db -> where($assign);
					...
					if($lead_type=='loan_type')
					{
					    $this -> db -> where('loan_type', $web_lead_name);
					}else
					{
		...
		//	echo $this->db->last_query();
				...
				//	echo $assignByRole;
					...
					elseif (in_array($assignToRole,$this->tl_array)) {
						//echo "testdata1";
...
// New Lead
	...
	$this -> db -> select('enq_id');
		...
		//If role CSE TL and CSE
		elseif ($assignByRole == '3' || $assignByRole=='2') {
			$this -> db -> where('l.cse_followup_id', 0);
...
//Pending new
	$this -> db -> select('enq_id');
		...
		elseif ($assignByRole == '16' || $assignByRole == '15') {
			...
			//$this -> db -> group_by('l.enq_id');
		}elseif ($assignByRole == '2' || $assignByRole == '3') {
			$this -> db -> where('l.cse_followup_id', 0);
			$this -> db -> where("l.assign_to_cse=", $assign_by);			
			$this -> db -> where('l.assign_to_dse_tl', 0);
			$this -> db -> where('l.assign_to_cse_date < ', $this -> today);
