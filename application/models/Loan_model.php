...
class Loan_model extends CI_Model {
	...
	public function insert_l_status($login_status) {
     //   $scheme_name=$this->input->post('l_status_name');
		$q = $this -> db -> query('select * from tbl_loan where  loan_name="'. $login_status .'" ') -> result();
		//print_r($q);

		if (count($q) > 0) {

			$query1=$this->db->query("select * from  tbl_loan where loan_name='$login_status'  AND  loan_status='-1'")->result();
			 if(count($query1)>0)
			 {
			 
			 $query=$this->db->query("update tbl_loan set loan_status='1' where loan_name='$login_status'    AND loan_status='-1' ");
			...
			$query = $this -> db -> query("insert into tbl_loan (`loan_name`,`loan_status`)values('$login_status','1')");
  ...
  $scheme_name=$this->input->post('scheme_name');
		$q = $this -> db -> query('select * from tbl_loan where loan_name="' . $reason_name . '" and  loan_id !="'.$login_status_id.'" ') -> result();
		...
		if (count($q) > 0) {
			...
			$this -> db -> query('update tbl_loan set loan_name="' . $reason_name . '"  where loan_id="' . $login_status_id . '"');
	...
	public function delete_l_status($id) {
		...
		$this -> db -> query('update tbl_loan set loan_status="-1" where loan_id="' . $id . '"');
		if ($this -> db -> affected_rows() > 0) {
