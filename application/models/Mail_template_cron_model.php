...
class Mail_template_cron_model extends CI_Model {
   ...
   public function select_fields() 
	 ...
	 public function insert_mail_template() 
	{
		...
		$query=$this->db->query("select * from tbl_mail_template where sms_type='$sms_type' and type='Email' and holiday_id='$holiday_id' and process_id='$this->process_id'")->result();

		if(count($query)>0)

         {	$query1=$this ->db->query("select * from tbl_mail_template where  sms_type='$sms_type' and type='Email' and holiday_id='$holiday_id'  AND status='-1' and process_id='$this->process_id' ")->result(); 

			 if(count($query1)>0)

			 {
			 $query=$this->db->query("update tbl_mail_template  set status='1'   where  sms_type='$sms_type' and type='Email' and holiday_id='$holiday_id'  AND status='-1' and process_id='$this->process_id'");
			 $t_id=$query1[0]->t_id;
			 }
		...
		else {	$query=$this->db->query("insert into tbl_mail_template (template_name,subject,description,status,created_time,created_date,process_id,sms_type,type,holiday_id)
			values('$name','$subject','$description','1','$this->time','$this->today','$this->process_id','$sms_type','Email','$holiday_id')");
...
//	print_r($images);
...

	{
        ...
        $holiday_id=$this->input->post('holiday_id');
		if($holiday_id==''){$holiday_id='0';}
		$query=$this->db->query("select * from tbl_mail_template where sms_type='$type' and type='Email' and holiday_id='$holiday_id' and status='1'  AND t_id !='$ctype_id' and process_id='$this->process_id'")->result();
	echo $this->db->last_query();
		if(count($query)>0)
         ...
         $this -> session -> set_flashdata('message', '<div class="alert alert-danger"><strong>Already Exists ...!</strong>	<a href="#" class="close" data-dismiss="alert" aria-label="close">×</a>	</div>');
			...
			$insert=$this->db->query("UPDATE tbl_mail_template set template_name= '$name', subject= '$subject', sms_type='$type',holiday_id='$holiday_id',description='$description' where t_id= '$ctype_id'");
            	 $i=0;
	    $dataid=1;
	    $t_id=$ctype_id;
	    	$images=array();
