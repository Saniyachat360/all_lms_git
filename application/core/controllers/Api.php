...
//For Add Customer
	...
	if($process_id==8){
		$query = $this -> api_model -> add_customer_evaluation($fname,$email,$address,$contact_no,$comment,$assign,$lead_source,$assignby,$username,$process_id,$location,$role_session,$user_id_session);
	}else{
		
		$query = $this -> api_model -> add_customer($fname,$email,$address,$contact_no,$comment,$assign,$lead_source,$assignby,$username,$process_id,$location,$role_session,$user_id_session);
...
public function select_lead_tracker() {
		...
		$query = $this -> api_model -> select_lead_tracker($process_name,$process_id,$campaign_name,$feedback,$nextaction,$date_type,$fromdate,$todate,$role_session,$user_id,$page);
		$query1 = $this -> api_model -> select_lead_tracker_count($process_name,$process_id,$campaign_name,$feedback,$nextaction,$date_type,$fromdate,$todate,$role_session,$user_id,$page);
		//$query1=count($query1);
...
public function select_all_followup_lead() {
	 $process_name= $this -> input -> post('process_name');
		...
		$query = $this -> api_model -> select_all_followup_lead($role,$process_name,$user_id,$contact_no,$page,$process_id);
		$query1 = $this -> api_model -> select_all_followup_lead_count($role,$process_name,$user_id,$contact_no,$page,$process_id);
		$response["lead_details_count"] = $query1;
		$response["lead_details"] = $query;
		echo json_encode($response);
	}
