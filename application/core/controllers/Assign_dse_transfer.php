...
class Assign_dse_transfer extends CI_Controller {

	function __construct() {
		parent::__construct();
		$this -> load -> library(array('table', 'form_validation', 'session'));
		$this -> load -> helper(array('form', 'url'));
		$this -> load -> model('assign_dse_transfer_model');
	...
	public function index() {
		...
		$data['from_location'] = $this -> assign_dse_transfer_model -> from_location();	
		$data['to_location'] = $this -> assign_dse_transfer_model -> to_location();		
		$data['var1'] = site_url('assign_dse_transfer/assign_data_admin');
		$this -> load -> view('include/admin_header.php');
		$this -> load -> view('assign_dse_transfer.php', $data);
	...
	function select_cse()
	{
		$dse_name=$this->assign_dse_transfer_model->select_cse();		
		if(count($dse_name)>0)
		{
			?>
	...
	function assigned1() {
		...
		$this -> session();		
		$query = $this -> assign_dse_transfer_model -> assign_data();
		if (!$query) {
	...
	function assign_data_admin()
	{
		...
		if($this->input->post('lead_type')=='status'){
			$query = $this -> assign_dse_transfer_model -> assign_data_status();
		}else{
			$query = $this -> assign_dse_transfer_model -> assign_data_source();
		}
	...
	//	redirect('assign_transferred/admin');
	}
	public function admin() 
	{
		...
		$data['from_location'] = $this -> assign_dse_transfer_model -> from_location();	
		$data['to_location'] = $this -> assign_dse_transfer_model -> to_location();		
		$data['var1'] = site_url('assign_transferred/assign_data_admin');
		$this -> load -> view('include/admin_header.php');
		$this -> load -> view('assign_transferred_leads_admin_view.php', $data);
		$this -> load -> view('include/footer.php');
	}
function select_fromuser()
	{
		$from_user=$this->assign_dse_transfer_model->select_fromuser();		
		if(count($from_user)>0)
		{
			...
			<div class="form-group">
				...
				<div class="col-md-9 col-sm-9 col-xs-8">
					<select name="fromUser" id="fromUser" class="form-control" required onchange="display_div()" >
			...
			<div class="col-md-9 col-sm-9 col-xs-8">
				No Records Found
				  <input type='hidden' id='fromUser'  class="form-control"  >
			</div>
			<?php
	...
	function checkLeads()
	{
		...
		if($lead_type=='source'){
		$all_count= $this -> assign_dse_transfer_model -> all_count_admin();
		$leads = $this -> assign_dse_transfer_model -> campaign_name();
...
if($lead_type=='status'){
		...
		$new_leads = $this -> assign_dse_transfer_model -> new_lead_counts();	
		$today_followup_leads = $this -> assign_dse_transfer_model -> today_followup_lead_counts();			
		$pending_new_leads = $this -> assign_dse_transfer_model -> pending_new_lead_counts();		
		$pending_followup_leads = $this -> assign_dse_transfer_model -> pending_followup_lead_counts();		
...
{
	$from_user=$this->assign_dse_transfer_model->select_touser();		
		if(count($from_user)>0)
		{
			?>
			<div class="form-group">
