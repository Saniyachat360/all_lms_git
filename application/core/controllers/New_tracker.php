...
public function download_complaint()
{
	...
	$from_date = $this -> input -> get('fromdate');
		...
		$csv="Sr No,Lead Source,Customer Name,Mobile Number,Alternate Mobile Number,Address,Email ID,Lead Date,Lead Time,Service Center,Customer Comment,Lead Assigned Date(CSE),Lead Assigned Time(CSE),CSE Name,CSE Call Date,CSE Call Time,CSE Call Status,CSE Feedback,CSE Next Action,CSE Remark,CSE NFD,CSE NFT,Auditor Name,Auditor call Date,Auditor call Time,Auditor call Status,Followup Pending,Call Received from Showroom,Fake Updation,Service Feedback,Auditor Remark\n";
	...
	foreach ($query as $row) {
		$i++;
			...
			if ($row -> lead_source == '') { $lead_source = "Web-".$row->business_area;
			} 
			...
			$csv.= $i.',"'.$lead_source.'","'.$row->name.'","'.$row->contact_no.'","'.$row->alternate_contact_no.'","'.$row->address.'","'.$row->email.'","'.$row->lead_date.'","'.$row->lead_time.'",'.$row->service_center.',"'.$row->comment.'",'.$row->assign_to_cse_date.','.$row->assign_to_cse_time.','.$cse_name.','.$row->cse_date.','.$row->cse_time.','.$row->csecontactibility.',"'.$row->csefeedback.'","'.$row->csenextAction.'","'.$row->cse_comment.'",'.$row->cse_nfd.','.$row->cse_nftime.',"'.$row -> auditfname . ' ' . $row -> auditlname .'",'.$row->auditor_date.','.$row -> auditor_time.','.$row -> auditor_call_status.','.$row -> followup_pending.','.$row -> call_received.','.$row -> fake_updation.','.$row -> service_feedback.','.$row -> auditor_remark."\n";
