...
ob_start();
class Escalation_dashboard extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model('escalation_model');
	...
	{
		$this->session();
		$data['select_location'] = $this->escalation_model->select_location();
		$this->load->view('include/admin_header.php');
		$this->load->view('escalation_view.php', $data);
...
?>
		...
		if ($location_name != '') {
			$location_data = $this->escalation_model->location_data($location_name);

			$data['location_data'] = $location_data;

			//$location_data=$this->escalation_model->location_wise_data($location_name);		
			if ($user == '') {
				$data['select_leads'] = $this->escalation_model->location_data($location_name);
				//print_r($data['select_leads']);
				$this->load->view('escalation_manager_view.php', $data);
			} else {
	...
	{
		$location_data = $data['location_data'] = $this->escalation_model->location_wise_data($location_name);
