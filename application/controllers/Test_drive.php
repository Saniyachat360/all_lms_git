...
class Test_drive extends CI_Controller {
	...
	function __construct() {
		...
		$this -> load -> model('test_drive_model');
	...
	public function leads() {
		$this -> session();
		$data['enq'] = 'Test Drive Today';
		//get pending attened leads details
		$id_array=$this->input->get('id_array');
		/*if(isset($id_array)){*/
		$data['select_lead'] = $this -> test_drive_model -> select_lead_location_wise();
		$data['test_drive_count_lead'] = $this -> test_drive_model -> select_lead_count_location_wise();
		/*}else{
