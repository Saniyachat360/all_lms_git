...
if($process_id == 8){
	...
	$enq_id=	$query = $this -> add_new_customer_model -> add_customer($fname, $email, $address, $pnum, $comment, $assign, $dept,$lead_source,$sub_lead_source,$process_id,$location,$process);
    if($process_id=='6'|| $process_id=='4'){
	if($enq_id !='0')
	{
	    $params= array(
           "enq_id" => $enq_id,
    ...
    $url = site_url('api_cron/add_new_lead');
    $o=$this->postCURL($url, $params);
  /* echo $o;
    $a=json_decode($o);
    print_r($a);*/
	 } }
