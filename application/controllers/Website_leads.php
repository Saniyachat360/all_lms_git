...
<?php
	...
	public function lms_details($id, $enq = null)
	{
		...
		$data['select_leads_flow'] = $this->website_lead_model->select_leads_flow($enq_id);
	...
	public function ebook_leads($type = null)
	{
		$this->session();
		if ($type == 'Insurance') {
			$lead_source = 'Insurance';
			$data['lead_source'] = $lead_source;
			$data['enq'] = 'Insurance Renewal';
		} else {
			$lead_source = '';
			$data['lead_source'] = $lead_source;
			$data['enq'] = 'EBook';
		}
		$t = $this->input->get("lead_source");
		if ($t == 'Insurance') {
			$lead_source = 'Insurance';
			$data['lead_source'] = $lead_source;
			$data['enq'] = 'Insurance Renewal';
		...
		$data['select_lead'] = $this->website_lead_model->select_ebook_lead($lead_source);
		$data['select_lead_count'] = $this->website_lead_model->select_ebook_count($lead_source);

		$data['id'] = '';
		$data['role'] = '';
		$this->load->view('include/admin_header.php');

		$this->load->view('ebook_filter_view.php', $data);
		$this->load->view('ebook_view.php');
		$this->load->view('include/footer.php');
	}
	public function ebook_filter()
	{
		$this->session();
		$data['enq'] = 'EBook';
		$enq = 'EBook';
		...
		$data['select_lead'] = $this->website_lead_model->select_ebook_lead($lead_source);
		$data['select_lead_count'] = $this->website_lead_model->select_ebook_count($lead_source);
		$this->load->view('ebook_filter.php', $data);
	}
	function insert_edbms()
	{
		$this->website_lead_model->insert_edbms();
		redirect('website_leads/ebook_leads');
	}
	public function ebook_leads_download()
	{
		...
		if ($lead_source == '') {
			$ffname = "EBook Payment";
		} else {
			$ffname = $lead_source . " Payment";
		}
		$query = $this->website_lead_model->ebook_lead_download();
		...
		$filename = $ffname . ' ' . $from_date . ' - ' . $to_date . '.csv';
