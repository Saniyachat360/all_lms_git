...
class Op_dse_productivity_tracker extends CI_Controller {

	function __construct() {
		parent::__construct();
		$this -> load -> library(array('table', 'form_validation', 'session'));
		$this -> load -> helper(array('form', 'url'));
		$this -> load -> model('op_dse_productivity_tracker_model');
	...
	function leads() {
		...
		$query = $this -> op_dse_productivity_tracker_model -> $source($from_date,$to_date,$dse_id);
		$source_count=$source.'_count';
		$data['select_lead'] = $query;
		$data['count_lead']=$this -> op_dse_productivity_tracker_model -> $source_count($from_date,$to_date,$dse_id);
		$data['id']=$dse_id;
		$enq = $source;
		$data['enq'] = ucwords((str_replace('_', ' ', $enq)));
		$data['tracker_name']='Op_dse_productivity_tracker';
