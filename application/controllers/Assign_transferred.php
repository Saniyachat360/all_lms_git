...
class Assign_transferred extends CI_Controller {
	...
	public function index() {
		$this -> session();
			...
			$data['user_name']=$this->assign_transferred_model->select_user();
	...
	function select_cse()
	{
		$dse_name=$this->assign_transferred_model->select_cse();		
		if(count($dse_name)>0)
		{
			?>
	...
	function assign_data_admin()
	{
		...
		if($this->input->post('lead_type')=='status'){
			$query = $this -> assign_transferred_model -> assign_data_status();
		}else{
			$query = $this -> assign_transferred_model -> assign_data_admin();
		}
	...
	{
		$this -> session();
		$data['from_location'] = $this -> assign_transferred_model -> from_location();	
		$data['to_location'] = $this -> assign_transferred_model -> to_location();		
...
function select_fromuser()
	{
		$from_user=$this->assign_transferred_model->select_fromuser();		
...
function checkLeads()
	{
		...
		if($lead_type=='loan_type'){
		$all_count= $this -> assign_transferred_model -> all_count_admin();
		$leads = $this -> assign_transferred_model -> campaign_name();
		...
		{			?>
			...
			<label class="control-label col-md-3 col-sm-3 col-xs-4" for="first-name"> </label>
			<div class="col-md-9 col-sm-9 col-xs-8">No Records Found
...
if($lead_type=='status'){
		...
		$new_leads = $this -> assign_transferred_model -> new_lead_counts();	
		$today_followup_leads = $this -> assign_transferred_model -> today_followup_lead_counts();			
		$pending_new_leads = $this -> assign_transferred_model -> pending_new_lead_counts();		
		$pending_followup_leads = $this -> assign_transferred_model -> pending_followup_lead_counts();		
...
{
	$from_user=$this->assign_transferred_model->select_touser();		
		if(count($from_user)>0)
		{
			?>
			<div class="form-group">
