...
ob_start();
class Notification_dash extends CI_Controller {

		function __construct() {
		parent::__construct();
		$this -> load -> library(array('table', 'form_validation','session'));
		$this -> load -> helper(array('form', 'url'));
		$this->load->model('new_notification_model_dash');	
		$this->load->model('complaint_notification_model_dash');	
	}
	...
	public function index() 
	{
		...
		$data['select_location']=$this->new_notification_model_dash->select_location();
		//print_r($data['select_location']);
		//$data['location_data']=$this->new_notification_model_dash->select_all_location_count();
		
		$data['var']=site_url('add_location/insert_location');
		$this->load->view('include/admin_header.php');
		$this -> load -> view('new_notification_view_dash.php',$data);
	...
	public function all_notification_counts()
	{
		...
		if($location_name!='')
		{
			$location_data=$this->new_notification_model_dash->location_data($location_name);	
			
			$data['location_data']=$location_data;	
				
			//$location_data=$this->new_notification_model_dash->location_wise_data($location_name);		
				if($user==''){
			$data['select_leads']=$this->new_notification_model_dash->location_data($location_name);	
			...
			?>
				...
				<tbody>
					...
					<tr>
						<td>1</td>
						<td><b>Home Visits For Today</b> </td>
					...
					<tr>
						<td>2</td>
						<td><b>Showroom Visits For Today </b></td>
					...
					<tr>
						<td>3</td>
						<td><b>Test Drives For Today </b></td>
...
$location_data=$data['location_data']=$this->new_notification_model_dash->location_wise_data($location_name);		
...
/****************************************** Complaint Functions *******************************************/
	public function all_notification_complaint_counts()
	{
		...
		if($location_name!='')
		{
			$location_data=$this->complaint_notification_model_dash->location_data($location_name);	
			
			$data['location_data']=$location_data;	
				
			//$location_data=$this->new_notification_model_dash->location_wise_data($location_name);		
				if($user==''){
			$data['select_leads']=$this->complaint_notification_model_dash->location_data($location_name);	
					?>
					<div class="col-md-12">
				
					<!--<a href="<?php echo site_url();?>notification_dash/send_mail/<?php echo $location_name;?>" class="btn btn-success pull-right">
								Notify GM
					</a>-->
