...
class unassign_leads extends CI_Controller {
	...
	public function downloadLeads()
	{
		...
		$query = $this -> unassigned_lead_model -> downloadLeads_e($feedback,$feedback1);			
		}
		...
		$csv= "Sr No,Interested In,Name,Mobile Number,Lead Date, Feedback Status, Next Action, Current User,Call Date, NFD, Remark\n";
		$i=0;
	foreach ($query as $row) {
		...
		$csv.= $i.',"'.$lead_source.'","'.$row->name.'","'.$row->contact_no.'","'.$row->created_date.'","'.$row->feedbackStatus.'","'.$row->nextAction.'","'.$currentUser.'","'.$callDate.'","'.$nfd.'","'.$comment.'"'."\n";

	}
	 //$csv_handler = fopen('php://output', 'w');
$csv_handler = fopen ('download_lead.csv','w');
fwrite ($csv_handler,$csv);
fclose ($csv_handler);	

$this->load->helper('download');
$filename = 'Lead- '. $filename.'.csv';
