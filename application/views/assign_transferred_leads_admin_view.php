...
<script>
				function select_fromuser()
		{
			...
			$.ajax({
				url:'<?php echo site_url('assign_transferred/select_fromuser')?>',
				type:'POST',
		...
		document.getElementById("leadsdiv").style.display='block';
			$.ajax({
				url:'<?php echo site_url('assign_transferred/checkLeads')?>',
				type:'POST',
				data:{fromUser:fromUser,lead_type:lead_type},
				success:function(response)
					...
					function select_touser()
		{
			...
			{
				$.ajax({
				url:'<?php echo site_url('assign_transferred/select_touser')?>',
				type:'POST',
			...
			<div class="panel panel-primary">
				...
				<div class="col-md-9 col-sm-9 col-xs-8">
					<select name="lead_type" id="lead_type" class="form-control" required onchange="checkLeads()">
                      <option value="">Please Select</option>
                      <option value="source">Lead Source Wise</option>
                      <option value="status">Status Wise</option>
                      <?php if($process_id=='1'){?>
                      <option value="loan_type">Loan Type Wise</option>
                      <?php } ?>
					</select>
			</div>
			</div>
