...
/*
    ...
    *
    */
function easycollect_payment($params, $redirect, $merchant_key, $salt, $env)
{
...
function _pay($params_array, $redirect, $salt_key, $url)
{
    ...
    $curl_result = _curlCall($url . 'easycollect/v1/create', http_build_query($params_array));
    ...
    if (empty($accesskey)) {
        ...
        $paymentdata =  $curl_result->data->payment_url;
        
         $email = $params_array['email'];
         $name =  $params_array['name'];
         $amount =  $params_array['amount'];
         $phone =  $params_array['phone'];
         $model_id =  $params_array['model_id'];
         $message =  $params_array['message'];
         ...
         header("Location: easy");
         
         //header("Location: easy?paymentdata=".$paymentdata.'&email='.$email.'&amount='.$amount.'&name='.$name.'&phone='.$phone.'&model_id='.$model_id.'&message='.$message);
...
{
    $hash_sequence = "key|merchant_txn|name|email|phone|amount|udf1|udf2|udf3|udf4|udf5|message";
...
{
    $reverse_hash_sequence = "message|udf5|udf4|udf3|udf2|udf1|amount|phone|email|name|merchant_txn|key";
