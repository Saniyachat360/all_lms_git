...
function test()
{
	...
	window.open(ur+"new_tracker_close/download_data/?date_type="+date_type+"&fromdate="+f+"&todate="+t+"&nextaction="+nextaction+"&feedback="+feedback+"&campaign_name="+c,"_blank");
}
</script>      
<script>
	...
	function getalldata () {
	  ...
	  var action= '<?php echo $nextaction ; ?>';
	  var fback= '<?php echo $feedback ; ?>';
	...
	function reset () { 
	 ...
	 window.location="<?php echo site_url();?>new_tracker_close/leads";
	}
</script>
<!-- Filter-->
...
<div class="row" >
  <?php if($_SESSION['process_id']==9)
	{?>
                   <form  action="<?php echo site_url(); ?>new_tracker_close/tracker_dse_filter_complaint" method="get" onsubmit="return validate_download();">     
     <?php }
	  elseif($_SESSION['process_id']==8)
		
			{
				if($_SESSION['sub_poc_purchase']==2)
		
			{?>
                <form  action="<?php echo site_url(); ?>new_tracker_close/tracker_sub_poc_purchase_filter" method="get" onsubmit="return validate_download();">     
		<?php }
		else{
			?>
                <form  action="<?php echo site_url(); ?>new_tracker_close/tracker_dse_filter" method="get" onsubmit="return validate_download();">     
		<?php
		}
				 }
  ...
  <?php
                         ...
                         <button type="submit" class="btn btn-info" ><i class="entypo-search"> Search</i></button>
                           ...
                           <!--  <a target="_blank" href="<?php echo site_url('new_tracker_close/download_data/?campaign_name='. $campaign_name .'&nextaction=' .$nextaction .'&feedback='.$feedback .'&fromdate='. $fromdate.'&todate='.$todate.'&date_type='.$date_type ); ?>" >--></a>
...
function select_next_action(feedback){
    ...
    $.ajax({url: "<?php echo site_url();?>new_tracker_close/select_next_action",
    type:'POST',
