...
<script>
	...
	function get_group() {
		...
		$.ajax({
			url: "<?php echo site_url(); ?>add_lms_user/get_group_name",
			type: "POST",
			data: {
		...
		<div class="panel panel-primary">
			...
			<div class="panel-body">
				<form action="<?php echo $var; ?>" method="post">
					<?php foreach ($select_location as $fetch) { ?>
						...
						<div id="demo-form2" data-parsley-validate class="form-horizontal form-label-left">
							<div class="col-md-12">
								<div class="form-group">
									<label class="control-label col-md-4 col-sm-4 col-xs-12" for="city">City: </label>
									<div class="col-md-5 col-sm-5 col-xs-12">
										<select class="form-control" id="city" name="city" required>
											<option value="">Select City</option>
											<option value="Pune" <?php echo ($fetch->city == 'Pune') ? 'selected' : ''; ?>>Pune</option>
											<option value="Mumbai" <?php echo ($fetch->city == 'Mumbai') ? 'selected' : ''; ?>>Mumbai</option>
										</select>
									</div>
								</div>
							</div>
