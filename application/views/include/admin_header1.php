...
//print_r($namearray);			
	/*	$rightarray=array($tracker_array,$new_lead_array,$assign_lead_array,$assign_transferred_lead_array);
		...
		{		
			...
			for($j=0;$j<$t1;$j++)
			{
				...
				if(isset($view[$viewvalue]))
				{
					if(($view[$viewvalue]==1 && $header_process_id==8))
					{
						//print_r($rightarray[$j]);
						 $namearray[$i]=1;}
					if($header_process_id==8)
					{
						//echo "hi";
	...
	//	}
              ...
              <!-- 	<li style='width:150px'>
               ...
               <div class="form-group" style="margin-top: 12px;margin-bottom: 0px;"> 
             	<select class="filter_s col-md-12 col-xs-12 form-control"  onchange="change_header_data1(this.value)" style="font-size: 12px;padding:6px 0px;">
						...
						<option value="<?php echo $header_location_id; ?>"><?php  echo $header_location; ?></option>
						 <?php
							...
							$query1 = $this->db->query("select l.location_id ,l.location 
									...
									where p.user_id='$header_user_id' and p.location_id !='$header_location_id'and p.process_id='".$header_process_id."' group by l.location_id")->result();
		...
		<?php	/* check rights of these four menus    process_id=>array_possition */
		$tracker_array = array(6=>13,7=>28,1=>38,4=>46,5=>54,8=>74,9=>80);	
		$new_lead_array = array(6=>1,7=>22,1=>32,4=>40,5=>48, 8=>68,9=>76);
		$assign_lead_array = array(6=>4,7=>23,1=>33,4=>41,5=>49,8=>69,9=>77);
		$assign_transferred_lead_array = array(6=>17, 7=>24,1=>34,4=>42,5=>50,8=>70,9=>77);		
				...
				{ 
					...
					<li class="dropdown">
						...
						<ul class="dropdown-menu">
							<!--<?php if(isset($view[58]))
							{ 
							if(($view[58]==1))
							{						
							?>
							<li>
								<a href="<?php echo site_url(); ?>reporting/leads?fromdate=<?php echo $daily_from_date; ?>&todate=<?php echo $daily_to_date; ?>&date_type=Lead">Daily Report</a>
							</li>
							<li class="divider"></li>
								<?php } } ?>
								<?php if(isset($view[59]))
							{ 
							if(($view[59]==1))
							{						
							?>
							<li>
								<a href="<?php echo site_url(); ?>reporting/leads?fromdate=<?php echo $monthly_from_date; ?>&todate=<?php echo $monthly_to_date; ?>&date_type=Lead">Monthly Report</a>
							</li>
							<li class="divider"></li>
								<?php } } ?>-->
							...
							<?php if(isset($view[60]))
							{
								if(($view[60]==1))
							{ 
							/*if(($view[60]==1 && $header_process_id==6) || ($view[60]==1 && $header_process_id==7))
							{*/						
							?>	
							<!--<li>
						...
						<?php 
					if(($header_process_id==6 ||$header_process_id==7 || $header_process_id==8)  && ($header_role==1 || $header_role==2))
					{
						 ?>
						 <li class="divider"></li>
						...
						<li class="divider"></li>
								...
								<ul class="dropdown-menu">
									<li>
									<a href="<?php echo site_url(); ?>CSE_reports"><span class="title">Productivity</span></a>
									</li>
									<li class="divider"></li>
									<li>
					...
					?><li class="divider"></li>
							<?php if($_SESSION['user_id']==1 || $_SESSION['user_id']==31){?>
					<li>
					...
					<li class="dropdown">
						...
						{ 
							...
							<li>
								...
								<?php }else {?>
								<a href="<?php echo site_url(); ?>add_new_customer"><span class="title">Add New Lead</span></a>
								<?php } ?>
							...
							<!-- Assign Leads -->
								...
								<?php /*if(isset($view[4]) || isset($view[23]) || isset($view[33])|| isset($view[41]) || isset($view[49]))
									{ 
										if(($view[4]==1 && $header_process_id==6) || ($view[23]==1 && $header_process_id==7) || ($view[33]==1 && $header_process_id==1) || ($view[41]==1 && $header_process_id==4) || ($view[49]==1 && $header_process_id==5))
										{*/	
										if($namearray[2]==1){					
										?>	
									<li>
