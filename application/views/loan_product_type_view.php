...
<h1 style="text-align:center;">Loan Product Type Report</h1>
...
<div class="col-md-12" id="loanstatus" style="display:none;">
    <div class="panel panel-primary">
        <div class="panel-body">
            <form action="<?php echo site_url(); ?>Loan_product_type/index" id="filterForm" method="POST">
...
function callIndexAjax(mumbaiChecked, puneChecked) {
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo site_url(); ?>Loan_product_type/index',
        data: {
            city: city
        },
        dataType: 'json',
        ...
        success: function(response) {
            ...
            if (response.success) {
                $('#table_div').show();
                populateLoanTable(response.loan_report, response.dates);
            } else {
...
function populateLoanTable(data, dates) {
    var table = $('#loantable');
    var thead = table.find('thead');
    var tbody = table.find('tbody');
    thead.empty();
    tbody.empty();
    var headerRow = '<tr><th>Loan Type</th>';
    ...
    data.forEach(function(item) {
        var row = '<tr><td>' + item.loan_type + '</td>';
        var statusTotal = 0;
        dates.forEach(function(date) {
...
function calculateGrandTotal(results, uniqueUserStatuses, dates) {
    ...
    $('#filterForm').submit(function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo site_url(); ?>Loan_product_type/filterSubmissionProcesses',
            data: formData,
            dataType: 'json',
            beforeSend: function() {
                $('#loader').show();
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    var tbodyHtml = '';
                    var uniqueUser = Object.keys(results[0]);
                    uniqueUser.splice(uniqueUser.indexOf('date'), 1);
                    var grandTotal = {};
                    uniqueUser.forEach(function(UserStatus) {
                        var row = '<tr><td>' + UserStatus + '</td>';
                        var statusTotal = 0;
                        dates.forEach(function(date) {
                            ...
                            var countValue = count ? parseInt(count[UserStatus]) : 0;
                        ...
                        grandTotal[UserStatus] = statusTotal;
                    });
    ...
    function exportToExcel() {
        ...
        XLSX.writeFile(wb, 'Loan_Type_Report.xlsx');
    }
</script>