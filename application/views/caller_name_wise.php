...
<h1 style="text-align:center;">Caller Wise Report</h1>
...
<div class="col-md-12" id="callerstatus" style="display:none;">
    <div class="panel panel-primary">
        <div class="panel-body">
            <form action="<?php echo site_url(); ?>CallerNameWise/index" id="filterForm" method="POST">
...
function callIndexAjax(mumbaiChecked, puneChecked) {
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo site_url(); ?>CallerNameWise/index',
        data: {
            city: city
        },
        dataType: 'json',
        ...
        success: function(response) {
            ...
            if (response.success) {
                $('#table_div').show();
                populateCallerTable(response.caller_report, response.dates);
            } else {
...
function populateCallerTable(data, dates) {
    ...
    thead.empty();
    tbody.empty();
    var headerRow = '<tr><th>Caller Name</th>';
    ...
    data.forEach(function(item) {
        var row = '<tr><td>' + item.callername + '</td>';
        var statusTotal = 0;
        dates.forEach(function(date) {
...
function calculateGrandTotal(results, uniqueUserStatuses, dates) {
    ...
    $('#filterForm').submit(function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo site_url(); ?>CallerNameWise/filterSubmissionProcesses',
            data: formData,
            dataType: 'json',
            beforeSend: function() {
                $('#loader').show();
    ...
    function exportToExcel() {
        ...
        XLSX.writeFile(wb, 'Caller_Name_Report.xlsx');
    }
</script>